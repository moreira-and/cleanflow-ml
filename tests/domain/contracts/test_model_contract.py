...
class ModelContract(ABC):
    ...
    def test_prepare_training_returns_summary(self, model_factory, valid_training_data):
        ...
        summary: TrainingSummary = model.prepare_training(
            ProblemType.CLASSIFICATION, valid_training_data
        )
    ...
    def test_train_then_predict_flow(
        self,
        model_factory,
        valid_training_data,
        valid_inference_data,
        prediction_output_checker,
    ):
        model: IModel = model_factory()
        train_summary: TrainingSummary = model.prepare_training(
            ProblemType.CLASSIFICATION, valid_training_data
        )
        model.train(
            ProblemType.CLASSIFICATION, valid_training_data, train_summary.config
        )

        pred_summary: PredictionSummary = model.prepare_prediction(valid_inference_data)
        output: ModelOutputData = model.predict(
            valid_inference_data, pred_summary.config
        )
        assert prediction_output_checker(output, valid_inference_data)

    def test_prediction_is_deterministic_if_expected(
        self, model_factory, valid_inference_data
    ):
        ...
        assert out1 == out2, (
            "Prediction outputs diverge for same input when determinism is expected"
        )

    def test_handles_invalid_input_gracefully(
        self, model_factory, invalid_inference_data
    ):
        model: IModel = model_factory()
        pred_summary: PredictionSummary = model.prepare_prediction(
            invalid_inference_data
        )
        try:
            _ = model.predict(invalid_inference_data, pred_summary.config)
        except Exception:
            pytest.skip(
                "Implementation raises domain-specific error for invalid input (acceptable)"
            )
