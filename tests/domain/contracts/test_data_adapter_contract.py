...
class DataAdapterContract(ABC):
    ...
    def test_prepare_transform_returns_summary(self, adapter_factory, sample_selected_data):
        adapter: IDataAdapter = adapter_factory()
    ...
    def test_transform_and_preserve_schema(self, adapter_factory, valid_selected_data, forward_preserving_checker):
        adapter: IDataAdapter = adapter_factory()
        summary: TransformationSummary = adapter.prepare_transform(valid_selected_data)
        model_input: ModelInputData = adapter.transform(valid_selected_data, summary.config)
    ...
    def test_inverse_transform_and_preserve(self, adapter_factory, model_output_data, inverse_preserving_checker):
        adapter: IDataAdapter = adapter_factory()
        inv_summary: InverseSummary = adapter.prepare_inverse(model_output_data)
        predicted: PredictedData = adapter.inverse_transform(model_output_data, inv_summary.config)
