...
from src.domain.interfaces.strategies.i_data_selector import IDataSelector, SelectionConfig, SelectionSummary
...
class DataSelectorContract(ABC):
    ...
    def test_prepare_returns_summary_with_config_and_observations(self, selector_factory, sample_cleaned_data):
    ...
    def test_select_applies_based_on_config(self, selector_factory, sample_cleaned_data, selection_preserving_checker):
        selector: IDataSelector = selector_factory()
        summary: SelectionSummary = selector.prepare(sample_cleaned_data)
    ...
    def test_idempotent_selection(self, selector_factory, valid_cleaned_data):
        ...
        assert first == second, "Selection should be idempotent on already selected data"

    def test_no_destructive_on_valid(self, selector_factory, valid_cleaned_data, selection_preserving_checker):
        selector: IDataSelector = selector_factory()
        summary: SelectionSummary = selector.prepare(valid_cleaned_data)
