...
def make_entity(data, schema=None):
    return BaseDataEntity(
        ...
        provenance=Provenance(
            source="unit_test", extraction_time=datetime.now(timezone.utc)
        ),
    )
...
def test_not_null_violation_detected():
    schema = DatasetSchema(columns=["a", "b"], constraints={"not_null": ["a", "b"]})
...
def test_range_constraint_detected():
    schema = DatasetSchema(columns=["score"], constraints={"range": {"score": (0, 10)}})
